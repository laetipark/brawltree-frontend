...
const UserBattleLogs = ({ id, type, mode }) => {
  ...
  const matchCount = userRecentBattles?.length || 0;
  const vicCount = userRecentBattles?.filter(({ MATCH_RES }) => MATCH_RES === -1).length || 0;
  const drwCount = userRecentBattles?.filter(({ MATCH_RES }) => MATCH_RES === 0).length || 0;
  const defCount = userRecentBattles?.filter(({ MATCH_RES }) => MATCH_RES === 1).length || 0;

  const artCount = userRecentBattles?.filter(({ BRAWLER_CL }) => BRAWLER_CL === 'Artillery').length || 0;
  const sinCount = userRecentBattles?.filter(({ BRAWLER_CL }) => BRAWLER_CL === 'Assassin').length || 0;
  const cntCount = userRecentBattles?.filter(({ BRAWLER_CL }) => BRAWLER_CL === 'Controller').length || 0;
  const dmgCount = userRecentBattles?.filter(({ BRAWLER_CL }) => BRAWLER_CL === 'Damage Dealer').length || 0;
  const mrkCount = userRecentBattles?.filter(({ BRAWLER_CL }) => BRAWLER_CL === 'Marksman').length || 0;
  const supCount = userRecentBattles?.filter(({ BRAWLER_CL }) => BRAWLER_CL === 'Support').length || 0;
  const tnkCount = userRecentBattles?.filter(({ BRAWLER_CL }) => BRAWLER_CL === 'Tank').length || 0;
